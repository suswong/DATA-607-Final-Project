...
# 
# data_2018_2022$crime <- ifelse(data_2018_2022$crime %in% c("FELONY SEX CRIMES","FORCIBLE TOUCHING","PROSTITUTION & RELATED OFFENSES", "SEX CRIMES"),"SEXUAL", data_2018_2022$crime)
# 
...
offense <- merge(x=offense2,y=offense, 
             by="crime", all.x=TRUE)
...
```{r, message=FALSE}
qw <- data_2018_2022 %>%
  filter(crime %in% c("ASSAULT 3 & RELATED OFFENSES","PETIT LARCENY","DANGEROUS DRUGS","FELONY ASSAULT","MISCELLANEOUS PENAL LAW" ,"VEHICLE AND TRAFFIC LAWS", "CRIMINAL MISCHIEF & RELATED OFFENSES" ,"ROBBERY", "GRAND LARCENY" , "DANGEROUS WEAPONS"))
...
Below is a table of the crime rate per 1000 people for each borough and year. In all years except 2019, Queens has the lowest crime rate per 1000 people. Bronx has the highest crime per 1000 people for all years except in 2021. 
...
In all years except 2019, Queens has the lowest crime rate per 1000 people. Bronx has the highest crime per 1000 people for all years except in 2021. 
...
ggplot(borough_by_year, aes(x = Year, y = crime_rate, colour = Borough)) +
  geom_smooth() + scale_y_continuous(labels = function(x) format(x, scientific = FALSE)) + ggtitle("Crime Rate Per 1000 People")
```
